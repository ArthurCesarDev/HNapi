    ...
    public function getMydocs(Request $request) {
        ...
        if($property) {
            ...
            if($unit > 0  ) { 
                ...
                $docs = Doc::where('id_unit', $property)
                ->orderBy('datecreat', 'DESC')
                ->orderBy('id', 'DESC')
                ->get();

                foreach($docs as $warnKey => $warnValue) {
                    $docs[$warnKey]['datecreat'] = date('d/m/Y', strtotime($warnValue['datecreat']));
                    $photoList = [];
